<template>
	<div v-if="isOpen" class="fixed right-4 bottom-0 w-64 z-50 hidden lg:block">
		...
		<ul
			class="relative max-w-64 z-30"
			:style="{ bottom: `${bottoms.length * offset - offset}px` }">
			<li
				...
				:style="{
					transform: `translateY(${bottoms[index]}px) scale(${1 - scales[index]})`,
				}"
				@mouseover="showTooltip(index)"
				@mouseleave="hideTooltip(index)">
				<svg
					t="1715015564979"
					class="icon mr-2 w-[24px] h-[24px]"
				...
				<p class="truncate select-none font-sans w-full">
					{{ item.message }}
				</p>
				<div
...
watch(
	...
	(newNotifications) => {
		if (newNotifications) {
			...
			tooltipVisible.value = new Array(newNotifications.length).fill(
				false,
			);
		}
	},
