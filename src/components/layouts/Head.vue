<template>
    <header
        ...
        <nav class="mx-auto flex h-full items-center justify-between select-none">
            <a class="font-bold text-xl" @click="router.push({ name: 'Home' })">数据管理系统</a>
            <div class="flex items-center gap-6">
                <div class="flex items-center text-sm gap-4">
                    <a class="text-gray-500 cursor-pointer hover:text-gray-400"
                        @click="router.push({ name: 'Home' })">主页</a>
                    <a class="text-gray-500 cursor-pointer hover:text-gray-400"
                        @click="router.push({ name: 'Presets' })">预设管理</a>
                </div>
                        ...
                        <div v-if="dropdownVisible"
                            ...
                            <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                                    role="menuitem" @click="logout">登出</a>
                            </div>
                        </div>
                    </transition>
                </div>
